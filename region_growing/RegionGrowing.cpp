...
// parameters
const int Threshold = 200; // valore della soglia
const uchar max_region_num = 100; // massimo numero di pixel in una regione
const double min_region_area_factor = 0.01; // minimo numero di pixel per determinare una regione
// L'8-intorno
...
int main(int argc, char **argv) {
    ...
    // Il resize viene fatto poiché è una procedura molto onerosa
    ...
    // Calcola la regione minima di interesse
    ...
    /**
     * Mask conterrà l'output della funzione di growing. 
     * Tutti i pixel che entrano a far parte della regione
     * corrente saranno a 1 nella mask
     * **/
        ...
        for (int y=0; y<src.rows; ++y) {
            if (dest.at<uchar>(cv::Point(x, y)) == 0) {
                ...
                if (mask_area > min_region_area) {
                    // mask * padding assegna un etichetta alla regione
                    dest = dest + mask * padding;   // record new region to "dest"
                    //cv::imshow("mask", mask * 255);
                    //cv::waitKey();
                    // Per evitare l'oversegmentazione
                    if(++padding > max_region_num) { printf("run out of max_region_num..."); return -1; }
                } else {
                    // La regione con etichetta 255 conterrà tutti i pixel
                    // che appartengono a regioni troppo piccole
...
void grow(cv::Mat& src, cv::Mat& dest, cv::Mat& mask, cv::Point seed, int threshold) {
    ...
    while(!point_stack.empty()) {
        ...
        for (int i=0; i<8; ++i) {
            ...
            // Se il pixel è esterno all'immagine
